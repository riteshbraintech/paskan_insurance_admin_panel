...
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">Create Article</div>
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a
                            href="{{ route('admin.banner.index') }}">Article Pages</a></li>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="p-4 border rounded">
                        <form action="{{ route('admin.article.store') }}" method="post" class="row g-3 needs-validation"
                            ...
                            @foreach (langueses() as $langCode => $language)
                                <div class="col-md-12">
                                    ...
                                    <textarea name="trans[{{ $langCode }}][content]" id="trans_{{ $langCode }}_content"
                                        class="form-control" cols="30" rows="10">{{ old('trans.' . $langCode . '.content') }}</textarea>
                                    @if ($errors->has('trans.' . $langCode . '.content'))
                                        <div class="text-danger">{{ $errors->first('trans.' . $langCode . '.content') }}
                                        </div>
                                    @endif
                                </div>
                            @endforeach
