                ...
                <tr data-id="{{ $item->id }}" class="sortable-row">
                    ...
                    <td>
                        <span
                            ...
                            onclick="changeStatus(event,{{ $item->id }})">{{ ucfirst($item->is_published == 1 ? 'Active' : 'Inactive') }}
                        </span>
                    </td>

                    <td>
                        <div class="table-actions d-flex align-items-center gap-3 fs-6">
                            <a href="{{ route('admin.insurances.view', ['id' => $item->id]) }}"
                            ...
                            <a href="{{ route('admin.insurances.edit', ['id' => $item->id]) }}" class="text-warning"
                                data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit"><i
                                    class="fa-solid fa-pen-to-square text-warning fs-5"></i></a>

                            <a href="javascript:void(0);"
                                onclick="deleteItem('{{ route('admin.insurances.delete', ['id' => $item->id]) }}')"
        ...
        jQuery(function($) {
            $("#sortable").sortable({
                ...
                update: function(event, ui) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.insurances.reorder') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            order: order
