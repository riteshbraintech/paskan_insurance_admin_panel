        ...
        <div class="card">
            <div class="card-body">
                <div class="p-4 border rounded">
                    <form id="technologyForm" method="post" class="row g-3 needs-validation" autocomplete="off">
                        @csrf
...
<script>
    ...
    $('#technologyForm').on('submit',function(event){
        event.preventDefault();
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status == true) {
                    ...
                    $('#technology').empty();
                    let option = `<option disabled value=''>Choose...</option>`;
                    response.data.forEach(function(value,key,array){
                        option += `<option ${value.id == response.id ? "selected" : ""} value="${value.slug}">${value.name}</option>`;
                    });
                   option += `<option value='other'>Other</option>`;

                   $.facebox.close();
                   $('.technology').html(option);
                }
