...
<aside class="sidebar-wrapper" data-simplebar="true" data-show="on">
    <div class="sidebar-header">
        <div>
            <img src="{{ loadAssets('images/bt-logo2.png') }}" class="logo-icon" alt="logo icon" style="width: 12rem; ">
        </div>
    ...
    <ul class="metismenu" id="menu">
        ...
        @if (in_array($role_id, [\App\Models\Role::ADMIN, \App\Models\Role::SUPERADMIN]))
            <li class="{{ $segment == 'staff' ? 'mm-active' : '' }}">
                <a href="javascript:;" class="has-arrow dropdown">
                    <div class="parent-icon"><i class="fa-solid fa-users"></i></i></div>
                    <div class="menu-title">Staff Account</div>
                </a>
                <ul class="{{ $segment == 'staff' ? 'mm-show mm-collapse' : '' }}">
                    <li class="{{ $segment . '/' . $child_segment == 'staff/create' ? 'mm-active' : '' }}"> 
                        <a href="{{ route('admin.staff.create') }}"><i class="fa-regular fa-circle"></i>Create Staff Account</a>
                    </li>

                    <li class="{{ $segment . '/' . $child_segment == 'staff/list' ? 'mm-active' : '' }}{{ $segment . '/' . $child_segment == 'staff/edit' ? 'mm-active' : '' }}"> 
                        <a href="{{ route('admin.staff.list') }}"><i class="fa-regular fa-circle"></i>All Staff Accounts</a>
                    </li>
                </ul>
            </li>
